...
class StationService:  # Changed from station_service to StationService
    ...
    def create(self, current_user):
        ...
        Returns the created station dict, or None if the operation was cancelled or failed.
        """
        clear_screen()
    ...
    def update(self, current_user) -> bool:
        """Update voting station information.

        Returns True if the station was successfully updated, False otherwise.
        """
        clear_screen()
    ...
    def delete(self, current_user) -> bool:
        ...
        Returns True if a station was deactivated, False otherwise.
        """
        clear_screen()
        ...
        if prompt(f"Confirm deactivation of '{self.store.voting_stations[sid]['name']}'? (yes/no): ").lower() == "yes":
            deleted_name = self.store.voting_stations[sid]["name"]
            self.store.voting_stations[sid]["is_active"] = False
            log_action(self.store, "DELETE_STATION", current_user["username"],
                      f"Deactivated station: {deleted_name} (ID: {sid})")
            print()
            success(f"Station '{deleted_name}' has been deactivated.")
            self.store.save_data()
            pause()
            return True
        else:
